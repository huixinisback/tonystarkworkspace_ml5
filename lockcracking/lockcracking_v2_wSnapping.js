...
function setup() {
    ...
    // game sprites
    dial = new Sprite(width / 2, height / 2, width*0.23, 'dynamic');
    dial.image = dialImg;
...
function draw() {
    ...
    let currentNumber = round(currentAngle/360*100);
    ...
    if (!cracked && inputDelay === 0 && direction !== 0 && directionChanged && input.length < 2) {
        ...
        input.push(Math.round(previousNumber/10)*10);
    ...
    if (hand){
        if(hand.keypoints[4] && hand.keypoints[8]){
            ...
            if (thumbSprite.overlapping(dialButton) && indexSprite.overlapping(dialButton) && dyThumb > 10 && dyIndex > 10){
                ...
                // Every 5° of hand rotation, rotate dial by 1
                while (accumulatedAngle[i] >= 5) {
                    dial.rotation += 3.6
                    accumulatedAngle[i] -= 5;
                }
                while (accumulatedAngle[i] <= -5) {
                    dial.rotation -= 3.6;
                    accumulatedAngle[i] += 5;
                ...
                // Convert current dial.rotation to a 0–100 number (normalized)
                let angle = ((dial.rotation % 360) + 360) % 360; // Normalize
                let number = round(map(angle, 0, 360, 0, 100)) % 100;

                // Snap number to nearest 10 based on direction, so it don't snap opposite .
                let snappedNum;
                if (direction === 1) {
                    snappedNum = Math.ceil(number / 10) * 10;
                } else {
                    snappedNum = Math.floor(number / 10) * 10;
                }

                // Wrap snapped number if over 99
                if (snappedNum >= 100) snappedNum = 0;

                // Convert snapped number to dial rotation angle
                // 0–50 → 0 to 180 (Clockwise), 51–99 → 0 to -180 (Anti-clockwise)
                let endGoal;
                if (snappedNum < 50) {
                ...
                if (snappedNum > 50)  {
                    endGoal = map(snappedNum, 100, 50, 0, -180); // 100 wraps to 0
                }

                // Lerp toward the computed endGoal angle
                let diff = endGoal - dial.rotation;
                if (abs(diff) < 0.0000000001) {
                    dial.rotation = endGoal;
                } else {
                    dial.rotation = lerp(dial.rotation, endGoal, 0.7);
                }
                handDetected[i] = false;
            }
        }
...
function hints(combo){
    ...
    if (number == combo[0]){
        ...
        case 0:
            ...
            if(isNaN(Number(hintSprite.text))){
                hintSprite.color = 'rgb(244, 0, 0)';
            }else{
